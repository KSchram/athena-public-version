...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int j=js; j<=je; j++) {
  for (int i=is; i<=ie; i++) {
    phydro->u(IDN,k,j,i) = d0; //density
    phydro->u(IM1,k,j,i) = -d0*v0*cos(pcoord->x1v(i))*sin(pcoord->x2v(j)); //y momentum, L = 1
    phydro->u(IM2,k,j,i) = d0*v0*sin(pcoord->x1v(i))*cos(pcoord->x2v(j)); //x momentum, L = 1
